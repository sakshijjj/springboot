...
pipeline {
  agent { label 'build' }
   environment { 
        registry = "sakshijoshi522/democicd" 
  ...
  stages {
    stage('Checkout') {
      steps {
        git branch: 'main', url: 'https://github.com/sakshijjj/springboot.git'
      }
    }
   ...
   stage('Stage VII: Scan Image ') {
      steps { 
        ...
        sh "trivy image --scanners vuln --offline-scan sakshijoshi522/democicd:latest > trivyresults.txt"
        }
    }
   ...
   stage('Stage VIII: Smoke Test ') {
      steps { 
        ...
        sh "docker run -d --name smokerun -p 8080:8080 sakshijoshi522/democicd"
